...
% ceil: 無條件進位
% img_final: video_imgs{i}
% img_initial: logo_img
% pts_final: interior_pts
% pts_initial: warped_logo_pts
pts_final = ceil(pts_final);
pts_initial = ceil(pts_initial);

% size(img_final,1) = 720 --> y
% size(img_final,2) = 1280 -> x
% pts_final(:,2) -----------> y -> row
% pts_final(:,1) -----------> x -> column
% [size(img_final,1), size(img_final,2)] = [720, 1280]
